...
export const Navbar: React.FC<NavbarProps> = ({ onToggleSidebar, onOpenSearch }) => {
  ...
  return (
    <header className="sticky top-0 z-20 w-full bg-white/90 backdrop-blur-md border-b border-slate-200/80 px-3 sm:px-6 py-2.5 flex items-center justify-between gap-2 sm:gap-4">
      {/* Left: Mobile Menu & Search Pill */}
      <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0 max-w-xl">
        <button
          onClick={onToggleSidebar}
          className="lg:hidden p-1.5 sm:p-2 text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition-colors shrink-0"
        ...
        <div className="lg:hidden flex items-center gap-1.5 shrink-0">
          <div className="w-6 h-6 rounded-lg bg-blue-600 text-white flex items-center justify-center font-heading font-extrabold text-[10px]">
            TMJ
          </div>
        </div>
        ...
        <button
          ...
          className="flex-1 flex items-center justify-between px-3 sm:px-3.5 py-1.5 text-xs text-slate-400 bg-slate-100/80 hover:bg-slate-100 border border-slate-200/70 rounded-full transition-all duration-150 group min-w-0"
        >
          <span className="flex items-center gap-1.5 sm:gap-2 text-slate-500 group-hover:text-slate-700 truncate">
            <Search className="w-3.5 h-3.5 text-slate-400 group-hover:text-blue-600 transition-colors shrink-0" />
            <span className="hidden sm:inline">Cari matkul, jadwal, materi, tugas...</span>
            <span className="sm:hidden text-[11px]">Cari sesuatu...</span>
          </span>
          <kbd className="hidden md:inline-block px-2 py-0.5 bg-white border border-slate-200 text-[10px] rounded-md font-mono text-slate-400 font-medium shadow-soft-xs shrink-0">
            Ctrl K
          </kbd>
        </button>
      </div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
        ...
        {classes.length > 0 && (
          <div className="relative">
            <button
              ...
              className={`flex items-center gap-1 px-2.5 sm:px-3 py-1.5 rounded-full text-xs font-heading font-semibold border transition-all ${
                isDevOrAdmin
            ...
            >
              ...
              <span className="truncate max-w-[80px] sm:max-w-none">
                {activeClass ? activeClass.name : "Kelas"}
              </span>
              {isDevOrAdmin && <ChevronDown className="w-3 h-3 text-slate-400 shrink-0" />}
            </button>
        ...
        {/* Role Pill (Desktop) */}
        <span
          className={`hidden md:inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-heading font-semibold border ${
            role === "developer"
        ...
        <div className="relative">
          ...
          {showNotifPopover && (
            <div className="absolute right-[-40px] sm:right-0 mt-2 w-[calc(100vw-32px)] max-w-sm sm:w-96 bg-white border border-slate-200 rounded-2xl shadow-soft-xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-100">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 p-1 hover:bg-slate-100 rounded-xl transition-colors shrink-0"
          >
            <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-600 text-white font-heading font-bold text-xs flex items-center justify-center shadow-soft-xs">
              {currentUser?.name ? currentUser.name.slice(0, 2).toUpperCase() : "TM"}
            </div>
          ...
          {/* Profile Menu */}
          {showProfileMenu && (
            <div className="absolute right-0 mt-2 w-52 sm:w-56 bg-white border border-slate-200 rounded-2xl shadow-soft-xl z-50 p-1.5 animate-in fade-in zoom-in-95 duration-100 text-xs">
